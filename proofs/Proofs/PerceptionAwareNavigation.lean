/-
  ...
  Zhang, Z. & Scaramuzza, D. ICRA 2534–2541 (IEEE, 2018).
...
The paper minimizes over a receding horizon H:
  ...
  J = Σ_{k=0}^{H-1} [ w_t * Δt_k + w_p * L_p(x_k) ] + J_terminal

where w_t ≥ 0 weights time, w_p ≥ 0 weights perception quality loss,
and L_p(x_k) ≥ 0 penalizes poor feature visibility.
...
theorem receding_horizon_cost_nonneg {H : ℕ}
    (dt : Fin H → ℝ) (Lp : Fin H → ℝ) (J_term : ℝ)
    (w_t w_p : ℝ) (hw_t : 0 ≤ w_t) (hw_p : 0 ≤ w_p)
    (h_dt : ∀ k, 0 ≤ dt k) (h_Lp : ∀ k, 0 ≤ Lp k) (h_term : 0 ≤ J_term) :
    0 ≤ ∑ k : Fin H, (w_t * dt k + w_p * Lp k) + J_term := by
...
theorem perception_quality_nonneg {n : ℕ} (visible : Fin n → Bool) :
    0 ≤ (∑ i : Fin n, if visible i then (1 : ℝ) else 0) := by
  apply sum_nonneg
  intro i _
  split_ifs <;> norm_num

/-- More visible features gives higher or equal perception quality -/
theorem perception_quality_monotone {n : ℕ}
    (vis1 vis2 : Fin n → Bool)
    (h : ∀ i, vis1 i = true → vis2 i = true) :
    ∑ i : Fin n, (if vis1 i then (1 : ℝ) else 0) ≤
    ∑ i : Fin n, (if vis2 i then (1 : ℝ) else 0) := by
  apply sum_le_sum
  intro i _
  split_ifs with h1 h2
  · exact le_refl 1
  · exact absurd (h i h1) (by simp [h2])
  · norm_num
  · exact le_refl 0
...
The paper enforces hard constraints on velocity and acceleration:
  ||v_k|| ≤ v_max  and  ||a_k|| ≤ a_max for all k in the horizon.
...
def vel_feasible {H : ℕ} (v : Fin H → ℝ) (v_max : ℝ) : Prop :=
  ∀ k : Fin H, |v k| ≤ v_max

/-- A trajectory is feasible if all acceleration norms are within bounds -/
def acc_feasible {H : ℕ} (a : Fin H → ℝ) (a_max : ℝ) : Prop :=
  ∀ k : Fin H, |a k| ≤ a_max

/-- Tightening the velocity bound keeps feasibility: if feasible at v_max,
    then feasible at any v_max' ≥ v_max -/
theorem vel_feasible_monotone {H : ℕ} (v : Fin H → ℝ) (v_max v_max' : ℝ)
    (h_le : v_max ≤ v_max') (hf : vel_feasible v v_max) :
...
theorem partial_sum_le_full {n m : ℕ} (hmn : n ≤ m) (f : ℕ → ℝ) (hf : ∀ k, 0 ≤ f k) :
